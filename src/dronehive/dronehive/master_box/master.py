...
class MasterBoxNode(Node):
	def __init__(self) -> None:
		...
		self.occupied = bool(self.config.drone_id)
	...
	def create_messages(self) -> None:
		...
		self.marker_pub = self.create_publisher(\
			Marker, 
			"/dronehive/box_markers", 
			10
		)
		self.master_occ_pub = self.create_publisher(
			Bool, 
			"/dronehive/master_occupied", 
			10
		)
	...
	def create_timers(self) -> None:
		...
		self.slave_publish_timer.cancel()
		# Timer to continuously update marker
...
nce
		...
		if closest_box_id != self.config.box_id:
			...
			self.slave_box_incoming_dron_pub.publish(String(data=f"{closest_box_id}:{request.drone_id}"))
			...
			try:
				...
				self.get_logger().info(
						f"Config updated: box_id='{self.box_id}' now occupied by drone_id='{self.config.drone_id}'."
					)
			except Exception as e:  # noqa: BLE001
				self.get_logger().warn(f"Failed to save config: {e}")
			
			self.occupied = True
			self._publish_master_occupied()
	...
	def handle_add_remove_drone_request(self,
	    ...
	    # ------------------------------------------------------------------
    # Timer: publish marker for master box
    # ------------------------------------------------------------------
	def _timer_cb(self) -> None:
        ...
        # UNIQUE per-master namespace
		marker.ns = f"box/master/{self.box_id}"
		marker.id = 0

		marker.type = Marker.CUBE
		marker.action = Marker.ADD

		lp: PositionMessage = self.config.landing_position
		marker.pose.position.x = float(lp.lat)
		marker.pose.position.y = float(lp.lon)
		marker.pose.position.z = 0.15  # centre of 0.3 m tall box
	...
	def _publish_master_occupied(self) -> None:
		msg = Bool()
		msg.data = self.occupied
		self.master_occ_pub.publish(msg)
