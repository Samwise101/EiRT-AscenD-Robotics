...
# -*- coding: utf-8 -*-
"""
DroneHive demo world: SCATTER (dual-path configs)

# These launch files write configs in BOTH places for each box:
#   1) <XDG_CONFIG_HOME>/dronehive/config.json   (what your dh.dronehive_initialise() is currently loading)
#   2) <XDG_CONFIG_HOME>/config.json            (what you *want* / what your slave_node docstring says)
#
# Master:
#   XDG_CONFIG_HOME = ~/.config
#   writes:
#     ~/.config/dronehive/config.json   (primary)
#     ~/.config/config.json            (extra)
#
# Slaves:
#   XDG_CONFIG_HOME = ~/.config/<slave_id>
#   writes:
#     ~/.config/<slave_id>/dronehive/config.json   (primary)
#     ~/.config/<slave_id>/config.json            (extra)
#
# We also write BOTH keys in the JSON:
#   - landing_position
#   - lending_position
# with identical values, to stay compatible with either schema.
"""
...
def _write_config_dual(home_dir: Path, rel_xdg: Path, payload: dict) -> None:
    ...
    # 1) What your current logs show dh.dronehive_initialise() uses:
    #    <XDG_CONFIG_HOME>/dronehive/config.json
    ...
    # 2) What you requested / what your slave docstring says:
    #    <XDG_CONFIG_HOME>/config.json
